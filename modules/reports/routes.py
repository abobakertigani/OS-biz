...
def register_routes(app, db):
    ...
    def dashboard():
        ...
        # 3. أكثر 5 عناصر مبيعًا
        top_items = []
        if MenuItemIngredient:
            # نحتاج لتحليل `items` من `Order` (محتواها JSON)
            # كحل مؤقت: نُظهر تقريرًا تقريبيًا
            ...
            top_items = [("لا توجد بيانات", 0)]  # مؤقت

        # 4. العناصر منخفضة المخزون
        ...
        # 5. الأرباح الصافية (تقديرية)
        # نحسب: الإيرادات - (تكلفة المواد المستهلكة)
        # كحل مؤقت: نُظهر الإيرادات فقط
        net_profit = round(daily_sales_result * 0.6, 2)  # تقدير: هامش ربح 40%
